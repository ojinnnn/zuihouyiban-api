...
type Config = {
   ...
   * - `crypto_mining`: Users must complete crypto mining tasks to obtain a
   *    temporary usertoken valid for a limited period.
   */
  captchaMode: "none" | "proof_of_work" | "crypto_mining";
  /**
...
export const config: Config = {
  port: getEnvWithDefault("PORT", 7861),
...
export async function assertConfigIsValid() {
  ...
  if (
    (config.captchaMode === "proof_of_work" || config.captchaMode === "crypto_mining") &&
    config.gatekeeper !== "user_token"
  ) {
    throw new Error(
      "Captcha modes 'proof_of_work' and 'crypto_mining' require gatekeeper mode 'user_token'."
    );
  }

  if (config.captchaMode === "proof_of_work" || config.captchaMode === "crypto_mining") {
